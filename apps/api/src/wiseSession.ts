...
export async function withWiseAccessToken<T>(
  tokenRef: string,
  ...
  if (shouldRefreshProactively(tokens)) {
    if (!tokens.refreshToken) {
      throw new WiseAuthError("Access token expired and no refresh token available", 401);
    }
  ...
  try {
    ...
    if (!(error instanceof WiseAuthError) || refreshed) {
      throw error;
    }
    if (!tokens.refreshToken) {
      throw error;
...
async function rotate(
  ...
  if (!current.refreshToken) {
    throw new WiseAuthError("Cannot refresh without refresh token", 401);
  }

  log?.info(
    { provider: current.provider, reason, expiresAt: current.expiresAt },
    "wise token refresh"
  );

  let response: WiseTokenResponse;
  try {
    response = await refreshWiseAccessToken({ refreshToken: current.refreshToken });
  } catch (error) {
    log?.warn(
      {
